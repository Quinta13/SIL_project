...
Although a slight trend over the years and some seasonality can be observed, these involve a very low magnitude of shootings, with only one or two shooting incidents.
...
The model explains approximately 11% of the variance, with a corrected R^2 that decreases to $`r round(summary(si_models$month)$adj.r.squared, 3)`$ due to the use of 12 predictors, one for each month, most of which are not statistically significant—meaning they do not significantly deviate from the annual mean. 

The only three significant predictors, along with the intercept, are February, May, and July, representing the months with the negative peak and two positive peaks of shootings throughout the year.
...
The qq plot is not entirely satisfactory as it tends to deviate in the tails. However, the Cook's distance plot highlights two leverage points that could be problematic.
...
Although the predictor associated with `DrugRelated` is significant, we proceed by adding a quadratic effect to the model for this reason.
...
L'effetto quadratico non sembra però portare alcun vantaggio al modello, anzi il modello sembra peggiorare con nessun predittore ora significativo.


#### 2.3.4. Stepwise regression

Now let's proceed with automatic predictor selection using stepwise regression and visualize the trend of adjusted R^2, AIC, and BIC metrics as the number of predictors varies, identifying the optimal number of predictors. (Note: among the candidate predictors, we exclude `TotArrests`, designed as perfectly collinear with all other crimes).
...
According to AIC, the predictors include `DrugRelated`, `ViolenceAndDisruption`, `Murder`, and the months of February, March, May, and July.
...
The only predictor that is not significant is the month of May, while all three predictors related to arrests for crimes are significant. However, it's important to note that the coefficient associated with `ViolenceAndDisruption` is negative, indicating a probable collinearity issue.
...
The model behaves similarly to the two models that separately modeled months and arrests, but overall improves the model quality in terms of R^2.
...
Now let's visualize the metrics such as adjusted R^2, AIC, and BIC for each model, accompanied by prediction metrics on the test set, specifically calculating the MSE and the average confidence interval for each prediction.
...
All predictors are significant, with the exception of October, which has a mean very close to the annual average. The QQ plot is not particularly satisfactory on the right tail, and the residual plot shows a slight quadratic curvature.
...
The residual plot consistently shows a slight quadratic effect compared to the fitted models. The QQ plot notably deteriorates on the right tail.

Let's compare the two models in terms of AIC, BIC, and adjusted R^2.
...
The models are generally comparable, with a BIC that favors the second one as it uses one-sixth of the parameters of the first model. AdjR2 slightly favors the first model, likely due to the seasonal phenomenon where shootings increase in December and decrease in January, which a model using trigonometric functions cannot capture.

However, BIC prefers the second model precisely because it uses fewer parameters compared to the first.

Let's look at the mean standard error in this example.
...
In general, the periodic model is also preferable in terms of uncertainty.

Let's visualize its predictions on the training set and test set.
...
All predictors, including the coefficients for the time polynomial, are highly significant.

The residual plot shows a quadratic effect for the fitted values, and the QQ plot is satisfactory on the right tail.

Let's visualize the prediction on the training set.
...
Including the number of arrests in the model significantly improves it across all considered metrics.

Let's now visualize if this new information helps the model predict changes in the training set.
...
The assumptions appear to be met with a prediction trend that manages to adapt to the trend over the years.

Let's now visualize the test set.
...
Once it's established how significantly the total number of arrests in the previous month improves predictions, it's appropriate to question if there are specific crimes that have a significant impact on shootings and if combining them could further enhance predictions. To do this, let's first visualize the R-squared of models using the time component and one crime at a time.
...
The crime that appears to perform best in this case is `ViolenceAndDisruption` with an R-squared of 0.3978. However, it's worth noting that unlike the previous model, `TotArrests` was considered without the quadratic effect.
...
vif_diagnostic(models$stepwise)
```
...
If the training set appears similar to the model with the total number of arrests, the prediction of the test set is certainly much more influenced by the predictors of the number of arrests, providing a more less smooth prediction.
...
- Only the boroughs of Brooklyn and Bronx, with Queens not selected but closely scaled to shootings in the reference level of Manhattan.
- The selected months are only February, September, and November.
- The most important crimes appear to be `Murder`, `Theft`, and `DrugRelated`, similar to those selected by stepwise.
...
Let's proceed to investigate the quality of all fitted models in terms of prediction metrics, AIC, BIC, and adjusted R-squared.
...
We have confirmed that the trend-modeling model shows strong signs of overfitting because, despite having the best scores for each metric, the model makes significantly higher prediction errors compared to other models. While the seasonal component alone is capable of explaining around 60% of the data variability, this can be improved by adding information about the number of crimes committed in the previous month.
...
Even the shrinkage model has a good MSE; however, the inability to calculate prediction uncertainties makes the model less reliable.

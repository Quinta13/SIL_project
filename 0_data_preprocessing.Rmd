...
- $`r length(unique(shootings_month$Crime))`$ different crime types in column `Crime`.
...
To make the dataset sutiable for merge with the shootings dataset, it is reshaped to create a column for each crime type. Moreover wea add a column with the total number of arrests.
...
We merge the two datasets to have a unique final wiew with the number of shootings and arrests for each borough, month and crime type. 

...
Before proceeding with the exploration, it is essential to adhere to the statistical and machine learning paradigm, which involves splitting the dataset into two parts: a training set and a test set. The test set simulates new, unseen data during the fitting phase and allows us to evaluate the model's performance on new data. 

For this reason, the exploration will focus solely on the training set.
...
This initial type of visualization provides an overview that can give a first idea of their magnitude variation:
...
- Some crimes show certain trends, such as `Theft`, which gradually increased starting in 2010 and then saw a decline from 2016. Another crime with a more erratic pattern is `Larceny`, which in 2014 appears to have experienced a rapid and drastic change in arrest numbers, followed by a reset in 2020.
- Arrests for `WeaponsPossession` have a very low number of arrests, with many months registering no arrests for this crime.
...
* The scatter plot involving the `Month` shows a clear seasonal dependence, with a peak in shootings during the summer months and a more pronounced decline in February. Additionally, boroughs with higher numbers of shootings also exhibit greater variability.
...
- Arrests for `LicensingFirearm` remain a crime with very low arrest numbers and consequently limited observations.
...
```{r chicago regions, fig.cap="NYC Boroighs", out.width = "370px"}
...
A potential issue that could arise during modeling is the presence of noisy data points, i.e., observations that significantly deviate from the distribution of other observations and could negatively affect the model. Since we are dealing with time series, we cannot simply eliminate outliers, but need a more sophisticated strategy: once identified, we can proceed to transform them to align more closely with the expected distribution, thereby reducing their modeling impact.
...
ALet's analyze in more detail the relationship among predictors with a VIF greater than 2.5.
...
- `DrugRelated` will combine MarijuanaPossession, SubstancePossession, and DrugParaphernalia, all closely related to drug sale and possession.
...
Una volta performata questa aggregazione, ricalcoliamo il VIF per verificare che la collinearità sia stata ridotta.
```{r}
